...
int Elf::LoadSymbols(std::vector<Syminfo *> &S_list, uint64_t base_addr){
    ...
    for(int i = 0; i < m_ehdr->e_shnum; i++){
        ...
        if(m_shdr[i].sh_type == SHT_SYMTAB){    // not dynsym because it only contains dynamic symbols whose addresses are set to 0
            ...
            for (int j = 0; j < m_shdr[i].sh_size / m_shdr[i].sh_entsize; j++){
                ...
                if(ELF64_ST_TYPE(sym[j].st_info) == STT_FUNC){
                    ...
                    Syminfo *syminfo = (Syminfo *) calloc(1, sizeof(Syminfo));
                    if(!syminfo){
