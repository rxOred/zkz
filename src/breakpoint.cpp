...
int BreakpointList::RemoveElement(const Debug& debug, int breakpoint_number){
    ...
    for(i = 0; i < B_List.size(); i++){
        ...
        if(B_List[i]->m_breakpoint_number == breakpoint_number){
            ...
            /* Restoring instruction at the address of the breakpoint we are removing */
            uint64_t data = ptrace(PTRACE_PEEKTEXT, debug.GetPid(), B_List[i]->m_address, nullptr);
            if((data & 0xcc) == 0xcc){

                if(ptrace(PTRACE_POKETEXT, debug.GetPid(), B_List[i]->m_address, B_List[i]->m_origdata) < 0){
...
Breakpoint *BreakpointList::GetElementByBreakpointNumber(int breakpoint_number) const{
...
void BreakpointList::ListBreakpoints(void) const{
    ...
    for(auto x = B_List.begin(); x !=  B_List.end(); x++){
        ...
        log.Print("\t%x\t%d\t\t%s\n", (*x)->m_address, (*x)->m_breakpoint_number, state);
    }
}
